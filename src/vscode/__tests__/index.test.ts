...
describe('VSCode UI-facing API', () => {
  ...
  it('keeps malformed JSONL log lines searchable as raw text', async () => {
    await withTempDir(async (cwd) => {
      await mkdir(join(cwd, '.omx', 'logs'), { recursive: true });
      await writeFile(
        join(cwd, '.omx', 'logs', 'broken.jsonl'),
        '{"timestamp":"2026-06-13T00:00:00.000Z","message":"unterminated\nplain fallback line with needle\n',
      );

      const results = await searchOmxLogs({ cwd, query: 'needle' });
      assert.equal(results.length, 1);
      assert.equal(results[0]?.message, 'plain fallback line with needle');
      assert.equal(results[0]?.source, 'omx');
    });
  });
