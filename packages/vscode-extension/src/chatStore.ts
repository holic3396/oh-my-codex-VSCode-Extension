...
function conversationPath(cwd: string, id: string): string {
  if (!/^[a-zA-Z0-9._-]+$/.test(id)) throw new Error(`invalid conversation id: ${id}`);
...
async function ensureDir(cwd: string): Promise<void> {
  await mkdir(conversationsDir(cwd), { recursive: true });
}

async function writeConversation(cwd: string, conversation: ChatConversation): Promise<void> {
  await ensureDir(cwd);
  const target = conversationPath(cwd, conversation.id);
  const temp = path.join(
    conversationsDir(cwd),
    `.${conversation.id}.${process.pid}.${randomUUID().slice(0, 8)}.tmp`,
  );
  try {
...
export async function readConversation(cwd: string, id: string): Promise<ChatConversation | null> {
  const file = conversationPath(cwd, id);
  let raw: string;
  try {
...
function parseConversation(raw: string): ChatConversation | null {
  let parsed: unknown;
  try {
...
function normalizeConversation(value: unknown): ChatConversation | null {
  ...
  if (parsed.schema_version !== 'omx.vscode/conversation/v1' || typeof parsed.id !== 'string') {
    return null;
  }
  return {
    schema_version: 'omx.vscode/conversation/v1',
